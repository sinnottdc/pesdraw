...
function drawfull() {
    ...
    this.drawEuro = function () {
        ...
        var data = {
            allTeams: [ france, germany, italy, netherlands, belgium, spain, portugal, greece, croatia, bosnia, england,
                switzerland, ukraine, russia, denmark, romania, scotland, sweden, serbia, turkey, hungary, czech, slovenia, austria, slovakia, albania,
                norway, iceland, poland, ireland, israel, bulgaria, northernIreland, wales ],
            seedOne: [ ],
            seedTwo: [ ],
            seedThree: [ ],
            seedFour: [ ],
    ...
    this.drawEuro2016 = function () {

        var data = {
            allTeams: [ france, germany, italy, netherlands, belgium, spain, portugal, greece, croatia, bosnia, england,
                switzerland, ukraine, russia, denmark, romania, scotland, sweden, serbia, turkey, hungary, czech, slovenia, austria, slovakia, iceland,
                norway, albania, poland, ireland, israel, bulgaria, northernIreland, wales ],
            seedOne: [ ],
            seedTwo: [ ],
            seedThree: [ ],
            seedFour: [ ],
        ...
        ( function () {
            //utils.shuffle(data.seedTwo);
            remainingSeedTwo = data.seedTwo.splice( 3, 4 ) //take 4 teams from seed 2 for round 3 & 4 draw (to avoid top seed picked in later rounds)
            var b = data.seedTwo.splice( 1, 2 ); // pick 2 team from seed 2 so possibility of 2nd seed team getting in no.1 position.
            var finalTeamsA = b.concat( data.seedOne ); // adding 2 teams from seed 2 to seed 1 array
            utils.shuffle( finalTeamsA ); // shuffle the array
            remainingTopSeeds = finalTeamsA.splice( 6, 4 ); // slice 4 teams and put them into position 1 of each group. Return remaining teams
            utils.shuffle( finalTeamsA ); // shuffle array for random positions else seed 2 will always go into group D if picked.
            draw( 6, finalTeamsA, 0 ); //position 1-4 draw}
            data.drawnTeams = data.drawnTeams.concat( finalTeamsA );
            s1 = utils.shuffle( finalTeamsA );
        } )();
        ...
        //positions 2 for each group - Containing remaining teams from seed 1 all teams from seed 2 & possibly 2 from seed 3
        ( function () {
            var b = data.seedThree.splice( 5, 4 ); //7 Add possible 2 teams from seed 3 top possibly be drawn in round 2 b = 2
            var c = remainingTopSeeds.concat( data.seedTwo ); //remainingTopSeeds = 4 ..seedTwo = 5  c = 9
            roundTwo = b.concat( c ); // roundTwo = 11
            utils.shuffle( roundTwo );
            var a = roundTwo.splice( 4, 6 );
            draw( 6, a, 1 );
            data.drawnTeams = data.drawnTeams.concat( a );
            s2 = utils.shuffle( a );
        } )();
        ...
        ( function () {
            remainingSeedThree = data.seedThree.splice( 2, 2 )
            b = remainingSeedTwo.concat( data.seedThree ); // 7 + 4
            utils.shuffle( b );
            var a = data.seedFour.splice( 6, 2 );
            seedThreeFinal = a.concat( b );
            utils.shuffle( seedThreeFinal );
            var c = seedThreeFinal.splice( 2, 6 );
            draw( 6, c, 2 );
            data.drawnTeams = data.drawnTeams.concat( c );
            s3 = utils.shuffle( c );
        } )();
    ...
    this.drawWorld = function () {
        ...
        var data = {
            allTeams: [ france, germany, italy, netherlands, belgium, spain, portugal, greece, croatia, bosnia, england,
                switzerland, ukraine, russia, denmark, romania, scotland, sweden, serbia, turkey, hungary, czech, slovenia, austria, slovakia, iceland,
                norway, albania, poland, ireland, israel, bulgaria, northernIreland, wales, brazil, argentina, colombia, uruguay, chile, costaRica, mexico,
                algeria, ivoryCoast, nigeria, ghana, egypt, tunisia, cameroon, burkinaFaso, guinea, mali, morocco, senegal, southAfrica, zambia, honduras, jamaica, panama, usa,
                bolivia, ecuador, paraguay, peru, venezuela, japan, southKorea, uae, australia, china, iraq, kuwait, lebanon, oman, northKorea, qatar, saudiArabia, thailand, newZealand, iran, uzbekistan, jordan ],
            seedOne: [ ],
            seedTwo: [ ],
            seedThree: [ ],
            seedFour: [ ],
        ...
        var euroSeedThree = [ albania, norway, iceland, poland, ireland, israel, bulgaria, northernIreland, wales, slovenia, slovakia ]; //7 9 teams
        ...
        utilsWC.sortArrays(data.allTeams);
        this.allTeams = data.allTeams;
        this.drawnTeams = data.drawnTeams;
    }
}
